...
bool OBJLoader::LoadOBJ(ID3D11Device* device, std::string filename, std::vector<Vertex::NormalTexVertex>& vertices, 
	std::vector<UINT>& indices, std::vector<MeshGeometry::Subset>& subsets, bool isRHS, bool uvFlipped)
{
			...
			for(int i = 0; i < 4; ++i)
			{
				...
				if(i == 3)
				{
					if(isRHS)
					{
						...
						subgroupIndices[activeSubgroup].push_back(subgroupIndices[activeSubgroup][subgroupSize - 2]);
						subgroupIndices[activeSubgroup].push_back(subgroupIndices[activeSubgroup][subgroupSize - 3]);
					}
					else
					{
						UINT subgroupSize = subgroupIndices[activeSubgroup].size();
						subgroupIndices[activeSubgroup].push_back(subgroupIndices[activeSubgroup][subgroupSize - 3]);
						subgroupIndices[activeSubgroup].push_back(subgroupIndices[activeSubgroup][subgroupSize - 1]);
					}
				}

				subgroupIndices[activeSubgroup].push_back(AddVertex(newVertex, vertices, hashTable, tempPosIndex));
	...
	for(std::map<UINT, HashEntry*>::iterator it = hashTable.begin(); it != hashTable.end(); ++it)
	{
	...
	for(int i = 0; i < subgroupIndices.size(); ++i)
	{
		...
		indices.insert(indices.end(), subgroupIndices[i].begin(), subgroupIndices[i].end());
	}

	return true;
}

UINT OBJLoader::AddVertex(Vertex::NormalTexVertex vertex, std::vector<Vertex::NormalTexVertex>& vertBuf, std::map<UINT, HashEntry*>& hashTable, UINT hashValue)
{
	HashEntry* currEntry = hashTable[hashValue];
	HashEntry* prevEntry = NULL;
	while(currEntry != NULL)
	{
		if(vertex.pos == vertBuf[currEntry->index].pos && vertex.tex == vertBuf[currEntry->index].tex && vertex.normal == vertBuf[currEntry->index].normal)
		{
...
void OBJLoader::LoadMTL(ID3D11Device* device, std::wstring filename, std::vector<MeshGeometry::Subset>& subsets)
{
	...
	while(fin >> symbol)
	{
		...
		else if(symbol == L"map_Kd")
		{
			...
			HR(D3DX11CreateShaderResourceViewFromFile(device, symbol.c_str(), 0, 0, &newData.srv, 0));
