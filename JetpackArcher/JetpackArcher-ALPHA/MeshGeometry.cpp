...
MeshGeometry::MeshGeometry(LitTexEffect* effect) : mEffect(effect)
{
}
...
void MeshGeometry::SetVertices(ID3D11Device* device, const Vertex::NormalTexVertex* verts, UINT count)
{
	ReleaseCOM(mVB);
...
void MeshGeometry::Draw(ID3D11DeviceContext* context, CXMMATRIX world, CXMMATRIX ITWorld, CXMMATRIX vp)
{
	...
	for(int i = 0; i <  mSubsetTable.size(); ++i)
	{
		...
		mEffect->Draw(context, mVB, mIB, mSubsetTable[i].faceStartIndex * 3, mSubsetTable[i].faceCount * 3);
	}
}